...
The body renderer (`render_body.py`) auto-detects a CJK-capable TTF and
remaps the body's `Times-Bold` / `Helvetica` slots to it so mixed
Chinese + English content renders correctly without per-character
fallback hacks.

Search order (first existing file wins):

1. `fonts/NotoSansSC-VF.ttf` — **bundled with this skill**. The skill is
   self-contained on any host that has `reportlab`; no font install
   required.  Variable font, ~17 MB, ReportLab loads the default axis
   (Thin).  Sits at `src/skills/minimax-pdf/fonts/`.
2. Windows: `C:/Windows/Fonts/simhei.ttf`
3. macOS: `/System/Library/Fonts/PingFang.ttc`
4. Linux: `/usr/share/fonts/.../NotoSansCJK-Regular.ttc`

If no CJK TTF is found, the renderer logs
`cjk_font_not_found` and falls back to Helvetica.  Chinese content will
appear as `.notdef` squares in that case — install a font or drop one
into `fonts/` to fix.

Notes on ReportLab quirks:

* **OTF/CFF outlines are not supported.** `TTFont` accepts only TrueType
  outlines.  The bundled font is `.ttf` for that reason.
* **Variable fonts load but only the default axis renders.** This is
  acceptable here — Noto Sans SC default (Thin) is legible and the
  body remains self-contained.
* **ToUnicode CMap is sparse.** ReportLab emits a minimal CMap for
  variable fonts; the visual glyphs are correct but `pypdf.extract_text`
  may return garbled output for body content.  The on-screen PDF is
  fine — only machine text extraction is affected.

To add a custom font, drop the TTF at `fonts/<name>.ttf` and prepend
`_SKILL_FONTS_DIR + "/<name>.ttf"` to `_CJK_CANDIDATES` in
