...
// ── Playwright loader (checks: local node_modules → global → global npm root) ─
function loadPlaywright() {
  const { execSync } = require("child_process");
  // ① Project-level node_modules (../../.. from this script to project root)
  try {
    const projRoot = path.resolve(__dirname, "..", "..", "..", "..");
    return require(path.join(projRoot, "node_modules", "playwright"));
  } catch (_) {}
  // ② Local require (works in some setups)
