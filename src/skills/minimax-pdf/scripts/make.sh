...
#
# ``python3`` is NOT a safe default on Windows: Git Bash finds the
# WindowsApps stub at ~/AppData/Local/Microsoft/WindowsApps/python3, which
# prints "Python was not found; run without arguments to install from the
# Microsoft Store" to stderr and exits non-zero.  That produced BUG-010
# (kg_make_pdf failing with an unhelpful error).
#
# Order: explicit $PDF_PYTHON override (set by PdfSkillRunner, which knows
# the interpreter actually running the server) → python3 → python.
# A candidate must be able to ``import sys`` to count as real, which
# filters out the Store stub.
_usable_python() {
  command -v "$1" &>/dev/null && "$1" -c "import sys" &>/dev/null
}

PY=""
for _cand in "${PDF_PYTHON:-}" python3 python; do
  [[ -n "$_cand" ]] || continue
  if _usable_python "$_cand"; then PY="$_cand"; break; fi
done
if [[ -z "$PY" ]]; then
  echo "ERROR: no usable Python interpreter found (tried \$PDF_PYTHON, python3, python)." >&2
  echo "       On Windows, 'python3' may be the Microsoft Store stub — set PDF_PYTHON to a real interpreter." >&2
  exit 2
fi
NODE="node"
...
cmd_check() {
  ...
  # Playwright — check: ① project-local node_modules  ② global require  ③ global npm root
  _proj_root="$(cd "$SCRIPTS/../../../.." && pwd)"
  if node -e "require('$PWD/node_modules/playwright')" 2>/dev/null || \
     node -e "require('$_proj_root/node_modules/playwright')" 2>/dev/null || \
     node -e "require('playwright')" 2>/dev/null || \
     node -e "require(require('child_process').execSync('npm root -g').toString().trim()+'/playwright')" 2>/dev/null; then
    green "  ✓ playwright"
  else
    yellow "  ⚠ playwright not found  (run: npm install && npx playwright install chromium)"
    ok=false
  fi
...
cmd_fix() {
  ...
  # Playwright — install as project dependency (not global)
  if command -v npm &>/dev/null; then
    ( cd "$_proj_root" && npm install --silent 2>/dev/null ) && \
    npx playwright install chromium --silent 2>/dev/null && \
    green "  ✓ Playwright + Chromium installed" || \
    { yellow "  playwright install failed — try: cd $(realpath "$_proj_root") && npm install && npx playwright install chromium"; rc=3; }
  else
