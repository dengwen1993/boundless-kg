...
# ── CJK font resolver (local, no network) ─────────────────────────────────────
# cover.py lives at <skill>/scripts/cover.py and fonts at <skill>/fonts/.
# Resolve once at import time so every CSS @font-face can reference the same path.
_SKILL_DIR    = Path(__file__).resolve().parent.parent
_CJK_FONT_CANDIDATES = [
    _SKILL_DIR / "fonts" / "NotoSansSC-VF.ttf",
    _SKILL_DIR / "fonts" / "NotoSansSC-Regular.ttf",
    _SKILL_DIR / "fonts" / "NotoSansCJKsc-Regular.otf",
]
_CJK_FONT_PATH = next((str(p) for p in _CJK_FONT_CANDIDATES if p.exists()), None)
_CJK_FONT_FAMILY = "Noto Sans SC"


def _cjk_font_face() -> str:
    """Return a CSS @font-face block for the local CJK font, or '' if missing."""
    if not _CJK_FONT_PATH:
        return ""
    # file:// URL with proper escaping (Chromium loads it from the same origin)
    abs_path = _CJK_FONT_PATH.replace("\\", "/")
    return (
        "@font-face { "
        f"font-family: '{_CJK_FONT_FAMILY}'; "
...
{_gfonts_import(t)}
{_cjk_font_face()}
/* CJK font goes FIRST so any Chinese characters pick up the local Noto Sans SC
   glyphs even if Google Fonts (Latin display) fails to load. */
* {{ margin: 0; padding: 0; box-sizing: border-box; }}
html, body {{
    width: 794px; height: 1123px;
    overflow: hidden;
    background: {t['cover_bg']};
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_body']}', 'Helvetica Neue', Helvetica, Arial, sans-serif;
}}
.page {{
    position: relative;
    width: 794px; height: 1123px;
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Times New Roman', Georgia, serif;
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Times New Roman', serif;
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', Georgia, 'Times New Roman', serif;
    font-weight: 300;
    font-size: 72px;
    line-height: 1.0;
    color: {text_dark};
...
.eyebrow {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', sans-serif;
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Helvetica Neue', sans-serif;
    font-weight: 900;
    font-size: 58px;
    line-height: 1.0;
    color: {text_l};
...
.ghost {{
    position: absolute;
    right: -60px; top: -40px;
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Arial Black', sans-serif;
    font-weight: 900;
    font-size: 680px;
    line-height: 1;
    color: {dark};
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Arial Black', Impact, sans-serif;
    font-weight: 900;
    font-size: 80px;
    line-height: 0.92;
    color: {body_color};
...
.subtitle {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', Georgia, serif;
...
.term-meta {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_body']}', 'Courier New', monospace;
...
.title {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_display']}', 'Courier New', monospace;
...
.author-label {{
    font-family: '{_CJK_FONT_FAMILY}', '{t['font_body']}', monospace;
