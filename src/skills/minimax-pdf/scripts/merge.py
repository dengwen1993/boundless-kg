...
# Make stdout tolerant of non-ASCII characters on legacy Windows
# code pages (e.g. GBK / cp936) so the QA summary below never crashes
# with UnicodeEncodeError just because the user double-clicked
# make.sh from cmd.exe.  ``errors="replace"`` substitutes ? for any
# un-encodable char; the JSON result above is unaffected.
...
def main():
    ...
    # Human-readable QA summary.  Kept pure ASCII so the summary renders
    # cleanly on Windows code pages (cp936 / GBK) without requiring the
    # user to `chcp 65001`.  Same visual hierarchy, portable glyphs.
    print("\n-- Build complete --------------------------------------")
