...
#: Absolute path of the ``fonts/`` directory shipped with this skill
#: (resolved relative to this script so the path is correct regardless of
#: cwd).  Bundled Noto Sans SC is searched first so the skill works on
#: hosts that don't have any CJK font installed.
_SKILL_FONTS_DIR = os.path.abspath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), os.pardir, "fonts")
)

#: Canonical name registered for the auto-detected CJK fallback font.
#: When a CJK-capable TTF is found on the system, ``register_fonts`` also
#: remaps the body's display / body / bold fonts to this name so every
#: ReportLab ``Paragraph`` automatically picks up a font that can render
#: Chinese / Japanese / Korean glyphs.
CJK_FONT_NAME = "MinimaxPdfCJK"

#: Search order for a CJK-capable TTF on disk.  ``simhei.ttf`` is preferred
#: because it is a plain TTF (one face) and ReportLab's TTFont handles it
#: directly without the .ttc sub-font gymnastics that ``msyh.ttc`` needs.
_CJK_CANDIDATES: tuple[str, ...] = (
    # 1. Skill-bundled Noto Sans SC variable TTF (covers Simplified
    #    Chinese + full Latin).  Sits next to the scripts so the PDF
    #    skill is self-contained on any host — no font install required.
    #    Resolved via ``_SKILL_FONTS_DIR`` below so the search stays
    #    correct even when cwd isn't the skill root.
    #    NOTE: variable fonts are ~17 MB; ReportLab loads the default
    #    instance (Thin axis).  PDF size trade-off for portability.
    _SKILL_FONTS_DIR + "/NotoSansSC-VF.ttf",
...
def _find_cjk_font_path() -> str | None:
    """Return the first existing CJK TTF path, or None."""
...
#: Set to True by :func:`register_fonts` once the CJK TTF is loaded.
_CJK_FONT_REGISTERED: bool = False


def register_fonts(tokens: dict):
    """Register fonts and (on CJK systems) remap body fonts.

    Why this remaps body fonts at all
    --------------------------------
    ``palette.py`` defaults to ``Times-Bold`` / ``Helvetica`` for body
    rendering because those are always available in ReportLab and produce
    reliable English output.  But neither ships with CJK glyphs, so any
    Chinese / Japanese / Korean content shows up as ``.notdef`` squares.

    Earlier versions tried monkey-patching ``Canvas.drawString`` /
    ``drawCentredString`` / ``drawRightString`` to per-character
    font-fallback.  That turned out to be insufficient because
    ``Paragraph`` renders via ReportLab's lower-level text-object API
    (``canv.beginText()`` etc.) which never goes through ``drawString``.
    A cleaner fix is to just install a CJK-capable font as the *body*
    font itself — SimHei (and similar) cover the full Latin range too,
    so a single font can render mixed content without any patching.

    Concretely, when a CJK TTF is detected we:

      1. Register it under :data:`CJK_FONT_NAME`.
      2. Mutate ``tokens["font_display_rl"]`` /
         ``tokens["font_body_rl"]`` / ``tokens["font_body_b_rl"]``
         in place so the existing ``ParagraphStyle`` instances (built
         from those keys just below) pick up SimHei automatically.

    The remap is local to the in-memory ``tokens`` dict — we never touch
    the on-disk palette defaults, so users who don't have CJK content
    (or don't have a CJK TTF) keep the original Times / Helvetica
    output unchanged.
    """
    global _CJK_FONT_REGISTERED

    # Caller-supplied TTF overrides (e.g. user pointed at their own font).
    ...
    if not cjk_path:
        # No CJK font available — keep Times / Helvetica as-is.  Mixed
        # content will show tofu for the CJK portion, but that's the
        # best we can do without a font to point at.
        print(
            json.dumps({"status": "warn",
                        "event": "cjk_font_not_found",
                        "hint": "install a CJK TTF (simhei / NotoSansCJK) "
                                "or pass --font-path to make.sh"}),
            file=sys.stderr,
        )
        return

    try:
        pdfmetrics.registerFont(TTFont(CJK_FONT_NAME, cjk_path))
    except Exception as e:
        print(
            json.dumps({"status": "warn",
                        "event": "cjk_font_failed",
                        "path": cjk_path,
    ...
    # Remap body fonts.  The keys below are the ones read by the
    # ParagraphStyle construction further down in this module.
    for key in ("font_display_rl", "font_body_rl", "font_body_b_rl",
                "font_heading", "font_body_b"):
        if key in tokens:
            tokens[key] = CJK_FONT_NAME
    print(
        json.dumps({"status": "ok", "event": "cjk_font_registered",
                    "name": CJK_FONT_NAME, "path": cjk_path,
                    "remapped": True}),
        file=sys.stderr,
    )
...
def make_styles(t: dict) -> dict:
    ...
    return {
        ...
        "body": ParagraphStyle("Body",
            ...
            spaceAfter=t["para_gap"], alignment=TA_LEFT, wordWrap="CJK",
        ),
        "bullet": ParagraphStyle("Bullet", wordWrap="CJK",
            fontName=bf, fontSize=t["size_body"],
            leading=t["line_gap"] - 1,
            textColor=HexColor(dk),
            spaceAfter=4, leftIndent=14,
        ),
        "numbered": ParagraphStyle("Numbered", wordWrap="CJK",
            fontName=bf, fontSize=t["size_body"],
            leading=t["line_gap"] - 1,
            textColor=HexColor(dk),
            spaceAfter=4, leftIndent=22, firstLineIndent=-22,
        ),
        "callout": ParagraphStyle("Callout", wordWrap="CJK",
            fontName=bfb, fontSize=t["size_body"] + 0.5, leading=16,
            textColor=HexColor(d),
        ),
        "caption": ParagraphStyle("Caption", wordWrap="CJK",
        ...
        "bib": ParagraphStyle("Bib", wordWrap="CJK",
            fontName=bf, fontSize=9, leading=14,
            textColor=HexColor(dk),
        ),
        "bib_title": ParagraphStyle("BibTitle", wordWrap="CJK",
            fontName=hf, fontSize=t["size_h2"],
            leading=t["size_h2"] * 1.4,
            textColor=HexColor(d),
...
def _add_table(story: list, item: dict, ctx: dict):
    ...
    # Optional col_widths. Accept either:
    #   - fractions that sum to ~1.0 (e.g. [0.2, 0.5, 0.3])  → multiplied by usable_w
    #   - absolute values in any unit (e.g. [1.5, 4.5, 3.5]) → auto-normalized
    #     to fractions so the table always fits the page width.
    if "col_widths" in item and len(item["col_widths"]) == n_cols:
        raw_cw = list(item["col_widths"])
        total = sum(raw_cw)
        if total > 1.5:  # Looks like absolute values, not fractions
            raw_cw = [w / total for w in raw_cw]
        col_w = [usable_w * f for f in raw_cw]
    else:
    ...
    # KeepTogether: if the table can't fit in the remaining space, push the
    # whole block to the next page. Without this, an oversized table at the
    # end of a page silently disappears.
    block = [tbl]
    if item.get("caption"):
        block.append(Spacer(1, 4))
        block.append(Paragraph(item["caption"], styles["caption"]))
...
def main():
    ...
    # Unpack wrapped content.json formats.
    # Accept either a bare list (legacy contract) or a dict with one of
    # the conventional wrapper keys. This makes the skill friendlier for
    # callers who naturally write {"blocks": [...]} / {"content": [...]} /
    # {"items": [...]} / {"chapters": [...]}.
    if isinstance(content, dict):
        for _key in ("blocks", "content", "items", "chapters"):
            _val = content.get(_key)
            ...
            print(
                json.dumps({
                    "status": "error",
                    "error": (
                        "content.json must be a list, or a dict with one "
                        "of these list-valued keys: blocks / content / "
                        "items / chapters"
                    ),
                }),
                file=sys.stderr,
    ...
    if not isinstance(content, list):
        print(
            json.dumps({
                "status": "error",
                "error": f"content.json must resolve to a list, got {type(content).__name__}",
            }),
            file=sys.stderr,
        )
        sys.exit(1)
